...
TrackHammer.swingCount = 2
TrackHammer.mayaFrameDuration = 1.0/30.0
TrackHammer.freezeDuration = 0.075

TrackHammer.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
TrackHammer.swingFrames = { 4.2 * TrackHammer.mayaFrameDuration, 4.2 * TrackHammer.mayaFrameDuration }
TrackHammer.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function TrackHammer.client_onCreate( self )
	self.isLocal = self.tool:isLocal()
	self:init()
end

function TrackHammer.client_onRefresh( self )
	self:init()
	self:loadAnimations()
end

function TrackHammer.init( self )
...
function TrackHammer.loadAnimations( self )
...
function TrackHammer.client_onUpdate( self, dt )
...
function TrackHammer.updateFreezeFrame( self, state, dt )
...
function TrackHammer.server_startEvent( self, params )
...
function TrackHammer.client_startLocalEvent( self, params )
	self:client_handleEvent( params )
end

function TrackHammer.client_handleEvent( self, params )
...
--function TrackHammer.sv_n_toggleTumble( self )
--	local character = self.tool:getOwner().character
--	character:setTumbling( not character:isTumbling() )
--end

function TrackHammer.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if self.pendingRaycastFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			if result.type == "body" then
				...
				for i=1,#shapes do
					self.network:sendToServer("server_SetRandomColor", shapes[i])
				end
			end
		end
	end
...
function GetBodyBoundingBox(body)
	minX, minY = 0, 0
	maxX, maxY = 0, 0
	shapes = body:getShapes()
	for i=1,#shapes do
		shape = shapes[i]
		pos = (shape:getLocalRotation() * shape:getWorldPosition()) -- sm.vec3.new(0.125,0.125,0.125)
		bounds = shape:getBoundingBox() / 2
		sign = sm.vec3.zero()
		if pos.x >= 0 then
			sign.x = 1
		else
		...
		origin = pos - bounds
		max = origin + (bounds * 2)
		if origin.x < minX then
			minX = origin.x
		end
		if origin.y < minY then
			minY = origin.y
		end
		if max.x > maxX then
			maxX = max.x
...
function TrackHammer.server_SetRandomColor(self, shape)
	shape:setColor(sm.color.new(math.random(), math.random(), math.random(), 1 ))
end

function TrackHammer.client_onEquip( self, animate )
...
function TrackHammer.client_onUnequip( self, animate )
